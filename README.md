...
- 어드민 `콘텐츠 관리` 신규 행의 마지막 칼럼을 `작업`으로 정리하고, `신규 저장` 버튼을 우측 고정 액션으로 노출해 가로 스크롤 없이도 바로 저장할 수 있게 개선했습니다.
- 콘텐츠 목록 기존 행도 마지막 액션 칼럼을 고정해 `열기/열림` 상태가 항상 보이도록 맞췄습니다.

### v0.1.5 (2026-06-07)

- 어드민 페이지에서 기존 `리액션 수정` 섹션을 제거하고, 카테고리/콘텐츠 관리 화면만 남기도록 정리했습니다.
- `GET /api/admin/dashboard` 응답에서 리액션 수정 전용 데이터를 제외하고, 관련 테스트 안내와 관리자용 리액션 수정 엔드포인트 문서를 함께 정리했습니다.
...
- 콘텐츠 마스터에 `releaseDate`, `searchKeywords`, `priorityScore`, `heroMessageKo`를 추가하고 관리자 화면에서 생성·수정할 수 있도록 정리했습니다.
- `editorial_slots`, `editorial_entries` 기반 대표 노출 구조와 우선 번역 검색 키워드 흐름을 연결해 요구사항 3 운영 시나리오를 지원합니다.
...
8. 콘텐츠 섹션에서 `+`로 신규 행 추가 후 마지막 `작업` 칼럼의 `신규 저장` 버튼으로 저장하고, 기존 행 클릭 시 섹션 아래 상세 패널이 열리는지 확인
...
- 신규 콘텐츠 행은 마지막 `작업` 칼럼의 `신규 저장` 버튼으로 `POST /api/admin/contents`를 호출하고, 저장 이후 상세 내용은 같은 섹션 아래 상세 패널에서 `PUT /api/admin/contents/:id`로 반영합니다.
